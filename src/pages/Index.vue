<template>
  ...
  <div class="col-4 d-flex row flex-center content-start">
    <q-input v-model="userToFind" @keyup.enter="searchUser" class="w-50" type="text" label="Nick do usuário" />
  ...
  <div v-if="userFound" class="d-flex column q-mt-md items-center">
    ...
    <p class="q-mt-md text-h4">{{userFound.name ? userFound.name : "Nome Não informado"}}</p>
    <p >{{userFound.email ? userFound.email : "Email não informado"}}</p>
    <p>{{userFound.bio ? userFound.bio : 'Bio não informada'}}</p>
    <p class="text-h6">Repositorios publicos: {{userFound.public_repos}}</p>
    <div class="row">

    <q-card @click="goToRepository(i)" v-for="(repository,i) in repositories" :key="i" class="q-ma-sm col-auto my-card">
      <div class="q-pa-sm d-flex column">
        <a class="text-h6">{{repository.name}}</a>
...
export default {
  ...
  methods: {
    ...
    async searchUser(){
      this.userFound = await this.$http(`/${this.userToFind}`).then(response => response.data)
      this.repositories = await this.$http(this.userFound.repos_url).then(response => response.data)
    }
  },
}
</script>
